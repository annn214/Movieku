...
export default class AuthController {
  ...
  async googleLogin({ request, response }: HttpContext) {
    ...
    if (!idToken) {
      return response.badRequest({ error: 'idToken wajib dikirim dari Google Identity' })
    }

    if (!GOOGLE_CLIENT_ID) {
      return response.internalServerError({ error: 'GOOGLE_CLIENT_ID belum dikonfigurasi di server' })
    }

    const verifyRes = await fetch(`https://oauth2.googleapis.com/tokeninfo?id_token=${idToken}`)
    if (!verifyRes.ok) {
      return response.unauthorized({ error: 'Token Google tidak valid' })
    }

    const payload = (await verifyRes.json()) as Record<string, any>

    if (payload.aud !== GOOGLE_CLIENT_ID) {
      return response.unauthorized({ error: 'Token tidak ditujukan untuk aplikasi ini' })
    }

    const email = payload.email as string | undefined
    const name = (payload.name as string | undefined) || 'Pengguna Google'

    if (!email) {
      return response.unauthorized({ error: 'Email Google tidak ditemukan pada token' })
    }

    let user = await User.findOne({ email })

    if (!user) {
      const passwordHash = await bcrypt.hash(idToken.slice(0, 32), 10)
      user = await User.create({ name, email, passwordHash, provider: 'google' })
    }
